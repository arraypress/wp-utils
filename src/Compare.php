...
if ( ! class_exists( 'Compare' ) ) :
	...
	class Compare {
		...
		/**
		 * Compare dates with relative terms.
		 *
		 * @param string $operator The comparison operator.
		 * @param string $value    The relative date string (e.g., '7 days ago', 'last month').
		 * @param string $date     The date to compare.
		 *
		 * @return bool
		 */
		public static function relative_date( string $operator, string $value, string $date ): bool {
			$relative_timestamp = strtotime( $value );
			$date_timestamp     = strtotime( $date );

			if ( $relative_timestamp === false || $date_timestamp === false ) {
				return false;
			}
