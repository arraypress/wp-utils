...
if ( ! class_exists( 'URL' ) ) :
	...
	class URL {
		...
		public static function is_audio( string $url ): bool {
			if ( self::check_url_type( $url, self::$audio_extensions, self::$audio_domains ) ) {
				return true;
			}
		...
		public static function is_video( string $url ): bool {
			if ( self::check_url_type( $url, self::$video_extensions, self::$video_domains ) ) {
				return true;
			}
		 ...
		 *
		 * @param string $url        The URL to check.
		 * @param array  $extensions Array of file extensions to check against.
		 * @param array  $domains    Array of domains to check against.
		 *
		 * @return bool True if the URL matches given extensions or domains, false otherwise.
		 */
		private static function check_url_type( string $url, array $extensions, array $domains ): bool {
			...
			return self::is_domain_match( $host, $domains );
		}
