...
class Extract {
	 ...
	 *
	 * @param mixed $object Object, object ID, or array with ID/id
	 *
	 * @return int|null The object ID or null if not found
	 */
	public static function object_id( $object ): ?int {
		// Already an integer
		if ( is_int( $object ) ) {
		...
		// Object with ID/id property
		if ( is_object( $object ) ) {
			// Check for common ID properties
			$id_properties = [ 'ID', 'id', 'Id', 'term_id', 'comment_ID', 'user_id' ];
			foreach ( $id_properties as $prop ) {
				if ( property_exists( $object, $prop ) ) {
					$id = absint( $object->$prop );
		...
		// Array with ID/id key
		if ( is_array( $object ) ) {
			$id_keys = [ 'ID', 'id', 'Id', 'term_id', 'comment_ID', 'user_id' ];
			foreach ( $id_keys as $key ) {
				if ( isset( $object[ $key ] ) ) {
					$id = absint( $object[ $key ] );
